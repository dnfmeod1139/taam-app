...
## 관리자 페이지 — 마이페이지와 합쳤다 (2026-08-31)

어드민·슈퍼어드민은 **마이페이지를 거치지 않는다.** GNB「My」가 곧장 콘솔 첫
화면(「오늘」)으로 간다. 종전엔 마이페이지 → 「Super Admin」 행 → 어드민 화면으로
화면 둘을 지났고, 그 사이의 마이페이지는 어드민에게 지나가는 화면일 뿐이었다.

| 역할 | GNB「My」 |
|---|---|
| `user` | 마이페이지 (**바뀐 것 없음**) |
| `admin` · `superadmin` | `openAdminConsole()` → 「오늘」 |

### 구조

```
adminScreen / partnerAdminScreen   ← 바닥. 전체 메뉴 + 하단 탭 바(acTabs·paTabs)
  └ 그 위에 콘솔 화면 다섯이 얹힌다 (.sub-screen, z 635)
      todayBoardScreen · rvScreen · tkbScreen · mbScreen · moScreen
      각자 하단에 같은 탭 바 (tbTabs·rvTabs·tkbTabs·mbTabs·moTabs)
```

- 탭: 슈퍼어드민 `오늘·예약·티켓·회원·더보기` / 매장 어드민 `오늘·예약·티켓·더보기`
- **매장 어드민에게도 「더보기」를 준다.** 없을 땐 「나의 레스토랑」·「예약 관리」가
  어느 탭 규칙에도 안 걸려 갈 곳이 없었다 (그래서 매장 화면만 접기를 못 했다)
- 마이페이지 개인 항목 = **「더보기」 맨 위 「내 정보」** (`_acMeHtml`·`acMy`)

### 건드릴 때 지킬 것

1. **어드민 메뉴 화면을 지우지 않는다.** 「더보기」가 그 DOM 을 훑어(`_moScan`)
   목록을 만든다. 닫아도 되지만 **DOM 에서 빼면 더보기가 통째로 빈다.**
2. **「내 정보」는 원래 페이지를 그대로 연다.** 화면을 새로 만들지 않는다 —
   `acMy()` 가 하는 일은 ①층 올리기(`.mp-over-ac`, z 660) ②원래 함수 호출뿐이다.
   회원 화면과 같은 코드가 돌아야 한 쪽만 고쳐지는 일이 안 생긴다.
3. **`.mp-subpage` 의 전역 z-index(620)를 올리지 않는다.** 콘솔에서 연 것만
   클래스를 달아 올린다. 마이페이지에서 여는 기존 흐름의 층은 그대로여야 한다.
4. **같은 항목을 두 곳에 놓지 않는다.** 탭으로 간 메뉴와 「로그아웃」은 더보기
   목록에서 뺀다 (검색할 때는 다 나온다 — 「여기 없습니다」로 답하면 안 된다).
5. **나가는 문과 화면 전환을 섞지 않는다.** `closePartnerAdmin()` 은 다른 화면을
   열기 직전에도 쓰인다(`openPartnerTU` 등). 복귀는 `exitPartnerAdmin()` 에 있다.
6. `_acFromMy` — 마이페이지를 거쳐 들어왔으면 나갈 때 그리로, 아니면 홈으로.
   직접 진입인데 마이페이지를 열면 없앤 그 한 단계가 나갈 때 되살아난다.
