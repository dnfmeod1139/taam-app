...
- **등급 판정 함수가 두 갈래다.** 상세 진입은 `getCurrentUserGrade()`(전화번호 →
  `memberDB`), 구매 시점은 `window._currentUserGrade`(프로필 기반)를 쓴다.
  `memberDB` 에 없는 M 회원이 M 전용 구간에서 상세만 막힐 수 있다 — 재현은 미확인.
...
3. **번역이 필요한 새 테이블·필드를 만들면 `I18N_AUTO_JOBS` 에 한 줄 추가한다.**
