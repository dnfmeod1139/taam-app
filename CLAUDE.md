...
## 사용자에게 코드·SQL 을 건네는 방법 (2026-08-30 합의)

사용자는 SQL Editor·대시보드에 **복사해서 붙여넣는** 방식으로 작업한다. 채팅에 긴
코드를 통째로 붙이면 스크롤에 묻히고 복사가 어긋난다. 그래서:

1. **긴 것(대략 30줄 이상) — 링크로 준다.** 저장소에 커밋·머지한 뒤 크롬에서 바로
   열리는 raw 링크를 준다:
   `https://github.com/dnfmeod1139/taam-app/raw/main/<경로>`
   (raw 는 파일 내용만 떠서 전체 선택 → 복사가 한 번에 된다)
2. **짧은 것(확인 쿼리 한두 개, 한 줄 수정) — 채팅에 직접** 붙인다. 판단 기준은
   「스크롤 없이 한 화면에 들어오는가」.
3. 링크를 줄 때는 **무엇을 어디에 붙여넣는지**(Supabase SQL Editor / Edge Function
   대시보드 / 기타)와 **몇 번 블록만 실행하는지**를 한 줄로 같이 적는다.
4. sql/·docs/·supabase/·*.md 만 바뀌는 커밋은 배포를 걸지 않는다
   (deploy.yml 의 paths-ignore) — 링크용 커밋이 Vercel 하루 한도를 먹지 않는다.
