...
mk(){ # purchase_id, created_at, reservation_date
  $P -c "set role authenticated; select set_config('taam.uid','$S',false);
    insert into public.deposit_transactions(user_id,deposit_type,change_type,amount,balance_after,metadata) values ('$U','general','ticket_purchase',-600000,0,'{\"purchase_id\":\"$1\"}');
    reset role;
...
mk P-FRESH "$(date -u +%FT%TZ)" "2027-01-01"
...
ok "경계: 정확히 D-31 은 환불된다" 500000 "$($P -c "insert into public.tickets(user_id,purchase_id,party_size,reservation_date,ticket_product_id,created_at) values ('$U','P-EDGE',2,to_char((now() at time zone 'Asia/Seoul')::date + 31,'YYYY.MM.DD'),'1','2026-09-01');
  insert into public.deposit_transactions(user_id,deposit_type,change_type,amount,balance_after,metadata) values ('$U','general','ticket_purchase',-600000,0,'{\"purchase_id\":\"P-EDGE\"}');
