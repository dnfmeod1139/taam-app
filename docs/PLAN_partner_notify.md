...
### 구멍 ② LINE userId 를 자동으로 못 받는다

LINE Messaging API 로 보내려면 그 사람의 `userId`(U로 시작)가 필요한데,
이건 **follow(친구 추가) webhook 으로만** 받을 수 있다. 그 webhook 이 없다.

지금은 LINE Developers 콘솔의 이벤트 로그에서 손으로 찾아 앱에 입력해야 한다.
매장 한두 곳이면 되지만, 파트너가 늘면 못 한다.
...
### 3-4. LINE userId 자동 수집 — `line-webhook`
...
## 5. 외부에서 먼저 시작해야 하는 것 — 리드타임이 있다

코드보다 **이쪽이 오래 걸린다.** 파트너십 일정이 정해져 있으면 지금 시작해야 한다.

| 무엇 | 걸리는 시간 | 비고 |
|---|---|---|
| 카카오 비즈니스 채널 + 비즈니스 인증 | 며칠 | `pfId` 가 여기서 나온다 |
| Solapi 가입 + 발신번호 등록 | 1일 | 발신번호는 통신사 확인이 붙는다 |
| **알림톡 템플릿 심사** | **1~3일** | ⚠ 결제 알림용 템플릿을 **새로** 올려야 한다. 지금 있는 건 예약 요청용 하나뿐 |
| LINE Official Account + Messaging API | 반나절 | 일본 매장용 |

**알림톡 템플릿이 병목이다.** 결제 알림 문구를 먼저 확정해서 심사에 올려두고,
그동안 코드를 만들면 기다리는 시간이 겹친다.
...
| 2 | 알림톡 결제 템플릿 심사 접수 | — | **가장 먼저**. 1~3일 걸린다 |
| 3 | `notify-purchase` Edge Function 작성·배포 | 대시보드 | 앱푸시만으로도 동작한다 |
| 4 | 앱에서 호출 붙이기 | 앱 1회 | `.catch` 로 실패를 삼킨다 |
| 5 | 실제 결제 1건으로 확인 | — | 파트너 계정 푸시가 오는지 |
| 6 | 템플릿 승인되면 시크릿 넣고 알림톡 확인 | — | 코드 변경 없음 |
| 7 | `line-webhook` + 연결코드 UI | 앱 1회 | 일본 매장 붙일 때 |

3·4 를 한 번에 하면 **앱 배포 1회**로 끝난다.
6 은 시크릿만 넣으면 되므로 배포가 필요 없다.
