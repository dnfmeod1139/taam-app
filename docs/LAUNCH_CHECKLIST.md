...
## 4-1. 출시 후 정리 작업 (지금 하지 말 것)

> **판단 시점: 토스 실결제 1건 검증 완료 후.** 그전에는 롤백 경로가 안전망이므로 그대로 둔다.

### 포트원(PortOne) 잔여 코드 제거

토스페이먼츠로 전환 확정, 다날 본인인증 미계약(응답 없음)으로 **포트원은 더 이상 쓰지 않는다.**
다만 아래 이유로 출시 전에는 건드리지 않는다.

- 죽은 코드 62곳은 4.85MB 파일에서 체감 이득이 없다 (위험 대비 실익 없음)
- `toss-confirm` 미구축 상태에서 롤백 경로를 먼저 없애는 건 순서가 거꾸로
- 92,000줄 단일 파일 + 테스트 없음 → 결제·인증 코드가 얽혀 있어 회귀 위험

**정리 대상**

| 항목 | 위치 | 비고 |
|---|---|---|
| PortOne SDK 로드 | `index.html` `<script src="https://cdn.portone.io/v2/browser-sdk.js">` | **실질 비용은 이것** — 매 실행 시 외부 CDN 블로킹 요청 |
| `PG_CHANNELS` / `PORTONE_STORE_ID` / `PORTONE_CHANNEL_KEY` | 결제 설정 블록 | |
| `PAY_MODE_TOSS === false` 분기의 구 결제 로직 | `doCardCharge()`, `openMpCharge()` 등 | 플래그와 함께 제거 |
| `startKoreanIdentityVerify()` (다날 경유) | 본인인증 | 라온 간편인증으로 대체 시 |

**주의** — SDK 태그 제거는 초기화 순서를 바꾼다. 스플래시/인트로 타이밍이 이 세션에서 여러 번 회귀했던 지점이므로, 제거 후 **비로그인 첫 진입 · 재진입 · 네이티브 앱** 3가지를 반드시 확인할 것.
