...
**ipa #35 (2026-08-27, 커밋 `f64ee77`) 가 최신 네이티브다.**

그 뒤로 네이티브에 영향 가는 변경은 **한 건뿐이고 안드로이드 전용**이다
...
- [ ] `app_config` 의 iOS 최신 빌드번호를 올린다 — **출시 확인 후에** 실행
...
> `min_build` 는 **0 그대로 둔다.** 1.01 도 푸시·결제가 정상이라 앱을 잠글 이유가 없다.
> 강제 안내는 보안·결제 사고처럼 구버전을 쓰게 두면 안 되는 때만 쓴다.
