...
## 2026-08-29 결정 — 1.02 는 그대로 내고, 수정은 1.03 으로

1.02 는 심사를 통과해 App Store Connect 에 **「배포 준비됨」** 으로 서 있다.
같은 표시 버전으로는 새 빌드를 못 올린다 ("The train version '1.02' is closed").
그리고 결제창이 앱 밖으로 나가는 것은 1.01 도 마찬가지라, 1.02 를 내는 것이
지금보다 나빠지는 일이 아니다 — 배지 자동 초기화·APNs·빌드 표식이 더해질 뿐이다.

그래서:

1. **1.02(#35)를 그대로 출시한다.** 심사를 통과한 빌드를 버리고 1~3일 더 기다릴 이유가 없다
2. `MARKETING_VERSION` 을 **1.03** 으로 올렸다. `allowNavigation` 이 들어간 빌드가 1.03 이 된다
3. 안드로이드도 1.03 으로 맞춘다 (두 스토어 표기를 갈라두지 않는다)

⚠ **1.02 에는 강제 업데이트를 걸지 않는다.** 곧 1.03 이 나오는데 연달아 강제하면
회원이 짧은 사이에 두 번 잠긴다. 1.02 는 `latest_build`(권장)만 올리고,
**강제(`min_build`)는 1.03 부터** 건다.
...
- [ ] **1.02 는 권장만 건다** — `sql/app_version_force_update.sql` 의 ② 블록에서
      `min_build` 는 **0 그대로 두고** `latest_build` 만 그 빌드번호로 올린다.
      곧 1.03 이 나오므로 연달아 강제하면 회원이 짧은 사이에 두 번 잠긴다.
- [ ] **강제(`min_build`)는 1.03 이 출시된 뒤에** ② 블록을 그대로 쓴다.
